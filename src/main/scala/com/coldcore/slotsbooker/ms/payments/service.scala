...
trait BookingMsRestCalls extends SystemRestCalls {
  ...
  def referenceFromMsBooking(ref: String, profileId: String): (Int, Option[vo.ext.Reference]) =
    restGet[vo.ext.Reference](s"$bookingBaseUrl/booking/reference?ref=$ref&profile_id=$profileId")

  def updateReferenceAsPaidWithMsBooking(ref: String, profileId: String): Int =
    restPatch[EmptyEntity](s"$bookingBaseUrl/booking/reference/paid", vo.ext.ReferencePaid(ref, profileId))._1

  def expiredReferenceFromMsBooking: (Int, Option[vo.ext.Reference]) =
    restGet[vo.ext.Reference](s"$bookingBaseUrl/booking/reference/expired")

  def cancelExpiredReferenceWithMsBooking(slotIds: Seq[String]): Int =
...
trait PaymentsService {
  ...
  def referenceByRef(ref: String, profileId: String): (Int, Option[vo.ext.Reference])
  def processReference(ref: String, profileId: String): Int
  def expiredReference(): (Int, Option[vo.ext.Reference])
...
trait Auxiliary {
  ...
  override def referenceByRef(ref: String, profileId: String): (Int, Option[vo.ext.Reference]) =
...
trait ProcessReference {
  ...
  override def processReference(ref: String, profileId: String): Int = {
    def step1(): Either[Int, vo.ext.Reference] = { // get reference
      val (code, reference) = referenceByRef(ref, profileId)
      if (reference.isEmpty) Left(code) else Right(reference.get)
    }

    def step2(reference: vo.ext.Reference): Either[Int, _] = // check status
      if (reference.quote.isDefined && reference.quote.get.status.get != quoteStatus('pending_payment)) Left(SC_CONFLICT)
      else if (reference.refund.isDefined && reference.refund.get.status.get != refundStatus('pending_payment)) Left(SC_CONFLICT)
      else Right(SC_OK)

    def step3(reference: vo.ext.Reference): Either[Int, Int] = // check if quote requires payment and check balance
      reference.quote.filter(_.amount.getOrElse(0) > 0) match {
        ...
        case Some(quote) =>
          ...
          if (place.isEmpty) Left(code)
            ...
            if (negative || credit >= required) Right(SC_OK) else Left(SC_CONFLICT)
          }
      }

    def step4(reference: vo.ext.Reference): Either[Int, _] = { // deduct or refund
    ...
    def step5(reference: vo.ext.Reference): Either[Int, _] = { // update reference
    ...
    if (eitherA.isRight) SC_OK
    else eitherA.left.get
  }

  override def expiredReference(): (Int, Option[vo.ext.Reference]) = {
